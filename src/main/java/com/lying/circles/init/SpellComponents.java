...
import com.lying.circles.magic.*;
import com.lying.circles.magic.ISpellComponent.Category;
import com.lying.circles.magic.component.*;
import com.lying.circles.magic.variable.*;
...
public class SpellComponents
{
	...
	{
		CircleMagic.LOG.info("# Reporting registered spell components #");
			for(Category cat : Category.values())
			{
				...
				{
					CircleMagic.LOG.info("# Added "+components.size()+" "+cat.name());
					components.forEach((id) -> CircleMagic.LOG.info("# * "+id.toString()));
				}
			}
		CircleMagic.LOG.info("# "+COMPONENTS.getEntries().size()+" total components #");
...
//		runComponentTests();
	...
	{
		CircleMagic.LOG.info("Running component tests");
		...
		CircleMagic.LOG.info("Circle index test: "+((VariableSigil)create(SIGIL_XYZ)).get(null).asDouble()+" runs = index "+testIndex.execute(new VariableSet()).get(Slot.BAST).asDouble());
	...
	private static void runArithmeticTests()
	{
		...
		CircleMagic.LOG.info("Add 4 + 2 test: "+testAdd.execute(new VariableSet()).get(Slot.BAST).asDouble());
		
		ISpellComponent testSub = create(GLYPH_SUB).addInputs(create(SIGIL_4), create(SIGIL_2)).addOutputs(create(Slot.BAST.glyph()));
		CircleMagic.LOG.info("Subtract 4 - 2 test: "+testSub.execute(new VariableSet()).get(Slot.BAST).asDouble());
		
		ISpellComponent testMul = create(GLYPH_MUL).addInputs(create(SIGIL_4), create(SIGIL_2)).addOutputs(create(Slot.BAST.glyph()));
		CircleMagic.LOG.info("Multiply 4 * 2 test: "+testMul.execute(new VariableSet()).get(Slot.BAST).asDouble());
		
		ISpellComponent testDiv = create(GLYPH_DIV).addInputs(create(SIGIL_4), create(SIGIL_2)).addOutputs(create(Slot.BAST.glyph()));
		CircleMagic.LOG.info("Divide 4 / 2 test: "+testDiv.execute(new VariableSet()).get(Slot.BAST).asDouble());
		...
		CircleMagic.LOG.info("Modulus 7 % 2 test: "+testMod.execute(new VariableSet()).get(Slot.BAST).asDouble());
	}
	
	private static void runLogicTests()
	{
		CircleMagic.LOG.info("2-bit adder tests:");
	...
	private static void runAdderTest(ResourceLocation bit0, ResourceLocation bit1, ResourceLocation bit2)
	{
		...
		CircleMagic.LOG.info(" * "+(var0 ? 1 : 0)+" + "+(var1 ? 1 : 0)+" ("+(var2 ? 1 : 0)+") = "+(int)variables.get(Slot.OSIRIS).asDouble()+" ("+(int)variables.get(Slot.RA).asDouble()+") "+(testPassed ? "PASSED" : "FAILED"));
	}
	...
	private static void runVectorTests()
	{
		...
		CircleMagic.LOG.info("Dot product of "+Direction.NORTH.getNormal().toShortString()+" and "+Direction.SOUTH.getNormal().toShortString()+": "+dotTest.execute(new VariableSet()).get(Slot.SOBEK).asDouble());
